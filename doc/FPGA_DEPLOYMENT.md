...
The Z-Core is a pipelined RISC-V RV32IM_Zicsr processor designed for educational purposes and FPGA deployment. It features an AXI-Lite bus interface connecting the CPU to memory and peripherals.
...
| `0x0400_3000 - 0x0400_3FFF`| 4 KB   | VGA        | 320×200 RGB332 VGA Controller     |
| `0x1000_0000 - 0x13FF_FFFF`| 64 MB  | SDRAM      | IS42S16320D (via AXI-CDC bridge)  |
...
>
> For programs larger than 12 KB (such as DOOM), use the 64 MB SDRAM at `0x1000_0000`. See [SOFTWARE.md](SOFTWARE.md) for the SDRAM build and upload flow.
...
The processor's instruction memory is initialized during FPGA synthesis. While applications are uploaded via UART, the initial bootloader is baked into the bitstream using Memory Initialization Files (`.mif`).
...
The top module specifies the initialization files:
...
make
```
This produces four `bootloader.mif` files in the `software/` directory. Quartus reads these files during synthesis to initialize the memory.
